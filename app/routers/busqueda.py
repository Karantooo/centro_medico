...
from ..repositories.busqueda_repository import (
    obtener_medicos,
    obtener_medicos_por_especialidad,
    obtener_medico_por_rut,
)
...
def mostrar_medicos_especialidad(especialidad: str):
    ...
    if not resultado:
        raise HTTPException(
            ...
            detail="No hay médicos disponibles con esta especialidad",
        )
    return {"data": resultado}

@router.get("/rut/{rut}", status_code=status.HTTP_200_OK)
def mostrar_medico_por_rut(rut: str):
    resultado = obtener_medico_por_rut(rut)
    if not resultado:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No hay un médico con ese rut"
        )
    return {"data": resultado}
